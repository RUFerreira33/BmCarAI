...
export class AiService {
  ...
  async extrairFiltros(mensagem: string): Promise<FiltrosExtraidos> {
    try {
      const resposta = await this.openai.chat.completions.create({
        model: process.env.OLLAMA_MODEL || 'llama3',
        temperature: 0, //mais previsível e menos criativo
        messages: [
          {
            role: 'system',
            content: `
...
Campos esperados:
{
  ...
  "observacoes": string | null
      ...
      const resultado = JSON.parse(jsonLimpo) as FiltrosExtraidos;
      ...
      resultado.combustivel = this.normalizeCombustivel(
        resultado.combustivel ?? this.extractCombustivelFromText(texto),
      );

      resultado.transmissao = this.normalizeTransmissao(
        resultado.transmissao ?? this.extractTransmissaoFromText(texto),
      );
      ...
      throw new InternalServerErrorException(
        'Erro ao extrair filtros com o modelo local.',
      );
    }
  }

  async recomendar(mensagem: string) {
    ...
    if (filtros.modelo) {
      ...
      resultados = resultados.filter((v: any) => {
        ...
        return (
          brand.includes(modeloPretendido) ||
    ...
    if (filtros.segmento) {
      ...
      resultados = resultados.filter((v: any) => {
        ...
        if (
          ['SUV', 'Carrinha', 'Berlina', 'Coupé'].includes(segmentoNormalizado)
        ) {
      ...
      resultados = resultados.filter(
        (v: any) => this.normalizeTipo(v.vehicleType) !== 'Moto',
      );
    }

    if (filtros.combustivel) {
      const combustivelNormalizado = this.normalizeCombustivel(
        filtros.combustivel,
      );
    ...
    if (filtros.transmissao) {
      const transmissaoNormalizada = this.normalizeTransmissao(
        filtros.transmissao,
      );

      resultados = resultados.filter((v: any) => {
        const transmission = this.normalizeTransmissao(
          v.transmission ?? v.gearbox ?? v.transmissao,
        );
  ...
  private normalizeTipo(input?: string | null): string | null {
    ...
    if (texto === 'moto') return 'Moto';
    if (texto === 'carro') return 'Carro';

    return input;
  }

  private normalizeSegmento(input?: string | null): string | null {
    if (!input) return null;
  ...
  private normalizeCombustivel(input?: string | null): string | null {
    if (!input) return null;

    const texto = input.toLowerCase().trim();

    if (
      texto.includes('gasóleo') ||
  ...
  private normalizeTransmissao(input?: string | null): string | null {
    if (!input) return null;

    const texto = input.toLowerCase().trim();

    if (
      texto.includes('automática') ||
  ...
  private extractCombustivelFromText(texto: string): string | null {
    if (
      texto.includes('gasóleo') ||
      texto.includes('gasoleo') ||
      texto.includes('diesel')
  ...
  private extractTransmissaoFromText(texto: string): string | null {
    if (
      texto.includes('automática') ||
      texto.includes('automatica') ||
  ...
  private normalizeTextoSimples(input?: string | null): string {
    if (!input) return '';
